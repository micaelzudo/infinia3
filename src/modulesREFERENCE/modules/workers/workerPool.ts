...
// --- Type Definitions --- 
// Define expected message type from worker
export type WorkerVertexDataMessage = [number, number, Float32Array]; 
...
function handleWorkerMessage(e: MessageEvent<WorkerVertexDataMessage>) {
    ...
    const chunkZ = e.data[1];
    const vertices = e.data[2]; // Received as Float32Array due to transfer
    ...
    if (_loadedChunks[chunkKey]) { 
        ...
        eventThrottled('worker-finish', 200, `🌱 Worker finished chunk [${chunkX}, ${chunkZ}] with Unified material`); 
        _scene.add(mesh);
        
        // Update loadedChunks - store mesh, discard noiseMap (no longer needed here)
        _loadedChunks[chunkKey] = { mesh: mesh, noiseMap: null }; 
    } else {
         (console as any).event(`❓ Worker finished chunk [${chunkX}, ${chunkZ}], but it was already unloaded.`);
