...
} from '../../debug/modules/ui/sketchbook/utils/sketchbookImports';
import type { ICharacterState } from '../../debug/modules/ui/sketchbook/entities/characters/states/characterStates';
import type { InputState } from '../../debug/modules/ui/sketchbook/core/input/inputTypes';
...
// Legacy character state interface for backward compatibility
...
let sketchbookCharacter: EnhancedCharacter | null = null;
let characterManager: CharacterManager | null = null;
let physicsManager: PhysicsManager | null = null;
let characterInputHandler: CharacterInputHandler | null = null;
...
// Initialize character reference with Sketchbook integration
const createCharacterReference = (inputManager: InputManager): CharacterRef => {
    // Create Sketchbook EnhancedCharacter
    const enhancedCharacter: EnhancedCharacter = {
        id: 'player-character',
        ...
        physics: {
            ...
            groundCheckRay: new THREE.Raycaster(),
            isGrounded: false,
            groundNormal: new THREE.Vector3(0, 1, 0),
            slopeAngle: 0,
            stepHeight: 0.3,
            stepCheckDistance: 0.5,
            slopeThreshold: 0.7,
            maxSlopeAngle: Math.PI / 4,
            collisionResponse: new THREE.Vector3(),
            stepClimbVelocity: new THREE.Vector3(),
            raycast: {
                hasHit: false,
        ...
        groundCheck: {
            raycastHitMethods: {
                raycast: () => false,
                getHitPoint: () => new THREE.Vector3(),
                getHitNormal: () => new THREE.Vector3(0, 1, 0),
                getHitDistance: () => 0
            }
        },
        states: {
        ...
        setState: function(stateName: string) {
            if (this.states.current !== stateName) {
                this.states.previous = this.states.current;
                this.states.current = stateName;
                this.states.timeInState = 0;
    ...
    // Initialize physics manager if not already created
    if (!physicsManager) {
        physicsManager = new PhysicsManager();
    }
    
    // Initialize the Sketchbook character
    initializeCharacter(enhancedCharacter, physicsManager);
    
    // Store reference
    sketchbookCharacter = enhancedCharacter;
    
    // Create legacy CharacterRef wrapper for backward compatibility
    const character: CharacterRef = {
        camera: new THREE.PerspectiveCamera(),
        gltf: null,
        mixer: enhancedCharacter.animations.mixer,
        currentAnimation: null,
        model: enhancedCharacter.modelContainer,
        physicsBody: enhancedCharacter.physics.body,
        inputManager,
        orientation: new THREE.Vector3(0, 0, 1),
        velocity: enhancedCharacter.velocity,
        stats: {
            walkSpeed: enhancedCharacter.moveSpeed,
            sprintSpeed: enhancedCharacter.sprintSpeed,
            jumpForce: enhancedCharacter.jumpForce
        },
        speedMultiplier: 1,
        avatarRadius: 0.5,
        avatarHeight: 1.8,
        ...
        // Methods - integrated with Sketchbook character system
        onInputChange(x: number, y: number) {
            // Delegate to Sketchbook character input handler
            if (characterInputHandler) {
                // Update input state based on x, y movement
        ...
        playAnimation(name: string, fadeInDuration = 0.2) {
            // Use Sketchbook animation system
            if (sketchbookCharacter && sketchbookCharacter.animations.mixer) {
                const actions = sketchbookCharacter.animations.actions;
                if (actions[name]) {
                    ...
                    sketchbookCharacter.animations.current = name;
                } else {
                    console.warn(`Animation ${name} not found in Sketchbook character`);
                }
            }
        ...
        stopAnimation(name: string) {
            // Use Sketchbook animation system
            if (sketchbookCharacter && sketchbookCharacter.animations.actions[name]) {
                const action = sketchbookCharacter.animations.actions[name];
                action.fadeOut(0.2);
            }
        ...
        setState: function(newState: CharacterState) {
            // Use Sketchbook character state system
            if (sketchbookCharacter && sketchbookCharacter.setState) {
                // Map legacy state names to Sketchbook states
                ...
                if (typeof newState === 'string' && stateMapping[newState]) {
                    sketchbookCharacter.setState(stateMapping[newState]);
                } else if (typeof newState === 'string') {
                    sketchbookCharacter.setState(newState);
                }
            }
...
// Initialize Sketchbook physics manager
if (!physicsManager) {
    physicsManager = new PhysicsManager();
    physicsManager.initialize();
}

// Initialize Sketchbook character manager
if (!characterManager) {
    characterManager = new CharacterManager(scene, physicsManager);
}
...
export function updateIsolatedThirdPerson(deltaTime: number): void {
    ...
    try {
        // Update Sketchbook physics manager
        if (physicsManager) {
            physicsManager.update(deltaTime);
        }
        
        // Update Sketchbook character manager
        if (characterManager) {
            characterManager.update(deltaTime);
        }
        
        // Update Sketchbook character
        if (sketchbookCharacter) {
            sketchbookCharacter.update(deltaTime);
        }
        
        // Update animation mixer (legacy support)
...
export function exitIsolatedThirdPerson(): void {
    ...
    // Clean up Sketchbook components
    if (sketchbookCharacter) {
        // Remove from scene if it has a model
        if (sketchbookCharacter.model && sceneRef) {
            sceneRef.remove(sketchbookCharacter.model);
        }
        sketchbookCharacter = null;
    }
