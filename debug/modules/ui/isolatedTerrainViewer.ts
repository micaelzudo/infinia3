...
// Rendering
import { createUnifiedPlanetMaterial, createInternalVisualizerMaterial } from '../rendering/materials';

// Utils
import { getChunkKeyY } from '../../utils_debug';
import { getMaterialIndexAtPoint_TS, createInternalMaterialGrid } from '../../../src/modules/rendering/InternalMaterialGrid';

// Yuka AI
import { initYuka, updateYuka, cleanupYuka, spawnYukaAgent } from '../ai/yukaController';
import { updateIsolatedFirstPerson } from './isolatedFirstPerson';
import { createYukaAIPanel, cleanupYukaAIPanel } from './yukaAiPanel';

// Procedural generation
import { 
    ProceduralGenerationSettings, 
    getProceduralGenerationSettings, 
    createProceduralGenerationPanel, 
    toggleProceduralGenerationPanel, 
...
} from "./trueBoundaryVisualizer";
import {
    addTheoreticalChunkBoundaries,
    removeTheoreticalChunkBoundaries
} from "./theoreticalBoundaryVisualizer";
import {
...
import { 
    ...
    addPostRenderHook,
    startAnimation
} from './isolatedTerrainViewer/core';
// Physics initialization and management
import {
    initPhysics,
    cleanupPhysics,
    updatePhysics,
    getPhysicsWorld
} from './isolatedTerrainViewer/physics';
import { setupEditing, cleanupEditing } from './isolatedTerrainViewer/terrainEditing';
import { LoadedChunks } from '../types';
import { genParams as aiGenParams } from '../ai/aiTerrainManager';
import { IsolatedTerrainLoader, createIsolatedTerrainLoader } from './isolatedTerrainLoader';
import { selectAgent } from '../ai/agentService';
...
declare global {
    interface Window {
        ...
        sketchbookWorldAdapterInstance?: any;
    }
}
...
let internalGridVisible = false;
let internalGridSettingsForViewer: InternalGridSettings | null = null;

// --- Yuka AI State ---
let yukaAiPanelElement: HTMLElement | null = null;
let isYukaInitialized = false;

// --- Follow Agent Camera State ---
let isFollowingAgent = false;
let followedAgent: THREE.Object3D | null = null;
let followCamera: THREE.PerspectiveCamera | null = null;
...
let proceduralGenerationPanelElement: HTMLElement | null = null;
let wildernessSurvivalPanelElement: HTMLElement | null = null;
let isWildernessPanelVisible = false;
...
let logCallback: ((keys: string[]) => void) | null = null;
...
let internalVisualizerLayers: THREE.Object3D[] = [];
...
// <<< ADDED: Module-level store for wireframe meshes >>>
let currentWireframeMeshes: { [key: string]: THREE.Mesh } = {};
let terrainLoader: IsolatedTerrainLoader | null = null;
...
// Internal grid controls
let internalGridControlsPanelElement: HTMLElement | null = null;
...
// Export function to get chunk meshes
export function getActiveChunkMeshesForCollision(): ChunkMeshesRef {
    return currentChunkMeshesForCollision;
}

// Export function to update chunk meshes
export function updateCurrentChunkMeshesForCollision(meshes: ChunkMeshesRef): void {
    currentChunkMeshesForCollision = { ...meshes };
}

// Initialize window object
if (typeof window !== 'undefined') {
    window.isolatedTerrainViewer = {
        ...window.isolatedTerrainViewer,
        currentChunkMeshesForCollision,
        updateCurrentChunkMeshesForCollision,
        terrainLoader: null // Will be set when terrain loader is created
    };
}

export function initIsolatedViewer(container: HTMLDivElement, onYukaPanelReady?: () => void) {
    if (!container) {
        console.error('[IsolatedTerrainViewer] Container element is required');
        return;
    }
    ...
    if (camera instanceof THREE.PerspectiveCamera) {
        followCamera = new THREE.PerspectiveCamera(camera.fov, camera.aspect, camera.near, camera.far);
        followCamera.position.set(0, 10, -20); // Initial position behind and above
        followCamera.lookAt(0, 0, 0); // Initial look at origin
    }
    ...
    if (scene && renderer) {
        terrainLoader = createIsolatedTerrainLoader(
            scene,
            renderer,
            currentNoiseLayers!,
            currentSeed!,
        ...
        // Expose terrain loader
        if (window.isolatedTerrainViewer) {
            window.isolatedTerrainViewer.terrainLoader = terrainLoader;
        }
    }

    // --- Remove aggressive OrbitControls event wrapping --- 
    // The previous approach of nullifying and re-adding listeners is removed.
    // We will rely on orbitControls.enabled and coordination with Sketchbook InputManager.

    // Add event listeners for terrain editing/interaction to the renderer's DOM element
    if (renderer) {
        renderer.domElement.addEventListener('mousemove', handleIsolatedMouseMove, false);
        renderer.domElement.addEventListener('mousedown', handleIsolatedEdit, false);
        renderer.domElement.addEventListener('contextmenu', handleIsolatedContextMenu, false);
    }

    // Add animation hooks
    addPreRenderHook(() => {
        if (!isYukaInitialized) return;
        updateYuka();
        updatePhysics(); // Add physics update to pre-render hook
    ...
    addPostRenderHook(() => {
        ...
        if (isFirstPersonMode) {
            ...
            const currentFPCamera = getIsolatedFirstPersonCamera();
            if (currentFPCamera && scene && renderer) {
                renderer.render(scene, currentFPCamera);
            }
        } else if (isThirdPersonMode) {
            updateIsolatedThirdPerson();
            const currentTPCamera = getIsolatedThirdPersonCamera();
            if (currentTPCamera && scene && renderer) {
    ...
    if (scene && containerElement && renderer && camera) {
        ...
        initYuka(scene, camera, renderer.domElement)
            .then(() => {
                ...
                if (containerElement) { // Ensure containerElement is not null
                    // Type assertions since we've already checked these are not null
                    const typedScene = scene as THREE.Scene;
                    const typedCamera = camera as THREE.Camera;
                    const typedRenderer = renderer as THREE.WebGLRenderer;
                    
                    yukaAiPanelElement = createYukaAIPanel(containerElement, {
                        scene: typedScene,
                        camera: typedCamera,
                        renderer: typedRenderer,
                        agent: null, // Will be set when agent is spawned
                        navMesh: null, // Will be set when navmesh is generated
                        chunkMeshes: {}, // Will be populated as chunks are loaded
                        spawnAgent: async () => {
                            if (typedScene && typedCamera) {
                                const spawnPos = typedCamera.position.clone().add(new THREE.Vector3(0, -2, -10).applyQuaternion(typedCamera.quaternion));
                                const agent = await spawnYukaAgent(spawnPos);
                                return agent;
                            }
                            throw new Error('Scene or Camera not available for spawning agent.');
                        }
                    });
                    console.log('Yuka AI Panel and Controller initialized by IsolatedTerrainViewer.');

                    // --- Re-attach event listener to Spawn Agent button --- NEW
                    if (yukaAiPanelElement) {
                        const spawnButton = yukaAiPanelElement.querySelector('button'); // Assuming the first button is Spawn Agent
                        if (spawnButton) {
                             // Optional: Remove existing listeners to prevent duplicates, though not strictly necessary if listeners are idempotent
                             // spawnButton.removeEventListener('click', ...);
                             spawnButton.addEventListener('click', async () => {
                                 console.log('[IsolatedTerrainViewer] Re-attached Spawn Agent button clicked.');
                                 if (typedScene && typedCamera && container) {
                                     const spawnPos = typedCamera.position.clone().add(new THREE.Vector3(0, -2, -10).applyQuaternion(typedCamera.quaternion));
                                     try {
                                         console.log('[IsolatedTerrainViewer] Spawning agent at:', spawnPos);
                                         const agent = await spawnYukaAgent(
                                             spawnPos,
                                             typedScene,
                                             typedCamera,
                                             container
                                         ...
                                         if (agent) {
                                             selectAgent(agent); // Explicitly select after spawning
                                             console.log('[IsolatedTerrainViewer] Agent spawned and selected via re-attached listener:', agent.name);
                                         }
                                     } catch (error) {
                                         console.error('[IsolatedTerrainViewer] Error spawning agent:', error);
                                     }
                                 } else {
                                     console.error('[IsolatedTerrainViewer] Scene, Camera, or Container not available for spawning agent:', {
                                         hasScene: !!typedScene,
                                         hasCamera: !!typedCamera,
                                         hasContainer: !!container
                                     });
                                 }
                             });
                             console.log('[IsolatedTerrainViewer] Re-attached Spawn Agent button listener.');
                        }
                    }

                    // --- Call the readiness callback --- NEW
                    if (onYukaPanelReady) {
                        onYukaPanelReady();
                    }
                }
            ...
            .catch(error => {
                console.error('Failed to initialize Yuka AI system:', error);
            });
    } else {
    ...
    setTimeout(() => {
        ...
        const spawnThirdPersonButton = document.getElementById('isolated-editor-spawn-third-person-button') as HTMLButtonElement;
...
function enterFirstPersonMode() {
    // --- Debug Logging --- NEW
    console.log('[DEBUG Camera] enterFirstPersonMode called.');
    // --- End Debug Logging ---
    if (!scene || !renderer /* removed !orbitControls here */) {
    ...
    // Check if pointer is already locked by another system (e.g. Sketchbook InputManager)
    if (document.pointerLockElement && document.pointerLockElement !== renderer?.domElement) {
        console.warn("Pointer already locked by another system, requesting release or aborting FP.");
        // Potentially try to request release from the other system if an interface exists
        // For now, we'll just prevent entering if lock is held by something else.
        if ((window as any).sketchbookInputManager && (document.pointerLockElement === document.body)) {
            console.log("Attempting to release pointer lock from Sketchbook InputManager (document.body)...");
            // We can't directly force it to unlock, but we can disable its desire for lock
            (window as any).sketchbookInputManager.setPointerLock(false);
             // Then try to exit its lock if it holds it - this might not work if it doesn't respond immediately
            if (document.pointerLockElement === document.body) document.exitPointerLock();
        } else {
            // If locked by something else unknown, or not sketchbook body, don't proceed.
            return;
        }
    }
    // If lock still exists after trying to release from sketchbook, wait or abort.
    // This part is tricky as exitPointerLock is async.
    // For now, let's assume the above setPointerLock(false) + exitPointerLock is enough
    // for our immediate attempt to lock renderer.domElement.

    // Tell Sketchbook InputManager to not interfere
    if ((window as any).sketchbookInputManager && typeof (window as any).sketchbookInputManager.setPointerLock === 'function') {
        console.log("[IsolatedTerrainViewer] Disabling pointer lock for Sketchbook.InputManager before entering FP mode.");
        (window as any).sketchbookInputManager.setPointerLock(false);
    }

    console.log("Entering Isolated First Person Mode...");
    isFirstPersonMode = true;
    isEditingEnabled = false; // Ensure editing is off when entering FP
    if (orbitControls) orbitControls.enabled = false;

    const fpInitResult = initIsolatedFirstPerson({
        scene: scene,
        renderer: renderer,
        terrainMesh: mainChunkMesh,
        loadedChunks: isolatedLoadedChunkData,
        chunkMeshes: {} as { [key: string]: THREE.Mesh | null },
    ...
    // Attempt pointer lock with error handling
    try {
        if (fpControlsRef) {
...
function exitFirstPersonMode() {
    if (!isFirstPersonMode || !camera) return; // Removed orbitControls from condition as it might be null
    ...
    document.removeEventListener('pointerlockchange', handlePointerLockChange); // Use named functions
    document.removeEventListener('pointerlockerror', handlePointerLockError);   // Use named functions
    ...
    // Restore Sketchbook InputManager's pointer lock preference
    if ((window as any).sketchbookInputManager && typeof (window as any).sketchbookInputManager.restoreInitialPointerLockState === 'function') {
        console.log("[IsolatedTerrainViewer] Restoring pointer lock for Sketchbook.InputManager after exiting FP mode.");
        (window as any).sketchbookInputManager.restoreInitialPointerLockState();
    }
}
...
function enterThirdPersonMode() {
    ...
    // Get SpacetimeDB context for multiplayer if available
    let spacetimeContext = null;
    let playerId = null;
    let enableMultiplayer = false;
    
    try {
        // Try to access SpacetimeDB context from the global scope or React context
        if (typeof window !== 'undefined' && (window as any).spacetimeDBContext) {
            spacetimeContext = (window as any).spacetimeDBContext;
            playerId = spacetimeContext.localPlayerId;
            enableMultiplayer = spacetimeContext.connection?.state === 'connected';
            console.log("[Terrain Viewer] SpacetimeDB context found, multiplayer enabled:", enableMultiplayer);
        } else {
            console.log("[Terrain Viewer] No SpacetimeDB context found, multiplayer disabled");
        }
    } catch (error) {
        console.warn("[Terrain Viewer] Error accessing SpacetimeDB context:", error);
    }
    ...
    try {
        const tpInitResult = initIsolatedThirdPerson({
            ...
            spacetimeDBContext: spacetimeContext,
            playerId: playerId,
...
export function generateIsolatedTerrain(params: GenerationParams) {
    ...
    // PATCH: Set aiTerrainManager.genParams for agent spawning
    aiGenParams.noiseLayers = params.noiseLayers;
    aiGenParams.seed = params.seed;
    aiGenParams.compInfo = params.compInfo;
    aiGenParams.noiseScale = params.noiseScale || 1.0;
    aiGenParams.planetOffset = params.planetOffset || new THREE.Vector3(0,0,0);
    ...
    if (typeof window !== 'undefined' && window.isolatedTerrainViewer) {
        window.isolatedTerrainViewer.currentChunkMeshesForCollision = currentChunkMeshesForCollision;
    }
    ...
    Promise.all(parallelGenerationPromises).then(() => {
        ...
        // FIX: Only use renderer and scene if both are not null
        if (renderer && scene) {
            // ... any code that uses renderer and scene ...
        }
    });
...
function regenerateAffectedMeshes(affectedKeys: string[]) {
    if (!scene || !currentCompInfo || currentNoiseScale === null || currentPlanetOffset === null || !physicsWorld) { // <<< ADDED Physics checks
        ...
        const getOrGenNeighborMap = (nx: number, ny: number, nz: number): NoiseMap | null => {
            ...
            if (generated) {
                ...
                if (generated.length !== CHUNK_HEIGHT + 1 || 
                    ...
                    console.error(`Generated noise map for ${neighborKey} has incorrect dimensions`);
                    return null;
                }
...
export function setupIsolatedEditing(
    ...
    // Tell Sketchbook InputManager to not interfere
    if ((window as any).sketchbookInputManager && typeof (window as any).sketchbookInputManager.setPointerLock === 'function') {
        console.log("[IsolatedTerrainViewer] Disabling pointer lock for Sketchbook.InputManager before terrain editing.");
        (window as any).sketchbookInputManager.setPointerLock(false);
    }
    
    isEditingEnabled = true; // Enable editing flag
    isFirstPersonMode = false; // Ensure FP is off
    if (orbitControls) orbitControls.enabled = false; // Disable OrbitControls when editing starts

    // Setup modularized terrain editing
    setupEditing(
        statusElement,
        logPanel,
        callback,
        isolatedLoadedChunkData as unknown as LoadedChunks,
        currentNoiseLayers!,
        currentSeed!,
        currentCompInfo!,
            currentNoiseScale!,
...
export function cleanupIsolatedEditing() {
    cleanupEditing();
}

// --- NEW MouseMove Handler for Brush AND Mining Hover Panel ---
function handleIsolatedMouseMove(event: MouseEvent) {
    if (!isEditingEnabled || !raycastBoxMesh || !editBrushMesh || !isPointerLocked) return;
...
function handleIsolatedEdit(event: MouseEvent) {
    if (isFirstPersonMode) return; // Do nothing if in FP mode

    if (isEditingEnabled) {
        // This event is for terrain editing. Prevent OrbitControls and other listeners.
        event.preventDefault(); 
        event.stopImmediatePropagation(); // More forceful than stopPropagation
                } else {
        // If editing is NOT enabled, don't interfere with OrbitControls or other potential click handlers.
        return;
    }
...
declare global {
  interface Window {
    ...
    analyzeVolumeComposition?: (
        ...
        // We might need to add resolutionContextWidth and resolutionContextHeight as optional parameters
...
function animate() {
    ...
    if (isYukaInitialized) {
        updateYuka(); // Update Yuka agents
    }
    updatePhysics(); // Add physics update to render loop
    ...
    if (isFirstPersonMode) {
        try {
            updateIsolatedFirstPerson();
            // Use the fpCameraRef that was set in enterFirstPersonMode
            if (fpCameraRef) {
                renderer.render(scene, fpCameraRef);
            } else if (camera) {
                console.warn('First-person camera not available, falling back to default camera');
                renderer.render(scene, camera);
            }
        } catch (e) {
            console.error('Error in first-person update:', e);
            // Fall back to default camera if first-person mode fails
            if (camera) {
                renderer.render(scene, camera);
            }
        }
    ...
    } else if (isFollowingAgent && followedAgent && followCamera) {
        // Calculate desired camera position and look-at target
        const targetPosition = followedAgent.position.clone().add(new THREE.Vector3(-5, 5, -10)); // Example offset: behind and above
        const lookAtTarget = followedAgent.position.clone();

        // Interpolate camera position (basic lerp)
        followCamera.position.lerp(targetPosition, 0.1); // Adjust interpolation factor (0.1 is 10% each frame)

        // Interpolate camera look-at (basic lerp on a temporary object for simplicity)
        const currentLookAt = new THREE.Vector3();
        followCamera.getWorldDirection(currentLookAt).negate(); // Get current look direction and negate to get target
        currentLookAt.add(followCamera.position);
        currentLookAt.lerp(lookAtTarget, 0.1);
        followCamera.lookAt(currentLookAt);

        // Ensure OrbitControls are disabled
        if (orbitControls) orbitControls.enabled = false;
        // Ensure FP/TP controls are disabled if they exist
        // (Assuming exitFirstPersonMode and exitThirdPersonMode handle disabling their controls)
        if (isFirstPersonMode) exitFirstPersonMode();
        if (isThirdPersonMode) exitThirdPersonMode();

        // Render with the follow camera
        renderer.render(scene, followCamera);
    } else { // OrbitControls mode
...
export function cleanupIsolatedViewer() {
    ...
    // Remove the call to the non-existent cleanupOrbitControlsListeners
    // if (cleanupOrbitControlsListeners) { // This variable itself is removed
    //     cleanupOrbitControlsListeners();
    //     cleanupOrbitControlsListeners = null; 
    // }
    ...
    if (renderer) {
         ...
         canvas.removeEventListener('mousemove', handleIsolatedMouseMove, false);
         canvas.removeEventListener('mousedown', handleIsolatedEdit, false);
         canvas.removeEventListener('contextmenu', handleIsolatedContextMenu, false); // Use named function for removal
    }
    ...
    if (renderer) {
        ...
        // renderer = null; // Commenting out, might be too aggressive if other modules share scene
    }
    camera = null;
    ...
    // Clean up wireframe meshes
    removeWireframeMeshes();
    currentWireframeMeshes = {}; // Ensure the map is empty after cleanup
}
...
// Keep the LogData interface and appendLogMessage function
interface LogData {
...
// Add core initialization check
function isCoreInitialized(): boolean {
    return !!(renderer && camera && scene && containerElement);
}

// Need to define these listeners to be removable
function handlePointerLockChange() {
    isPointerLocked = document.pointerLockElement === renderer?.domElement;
    if (!isPointerLocked && isFirstPersonMode) {
        exitFirstPersonMode();
    }
    if (!isPointerLocked && isEditingEnabled) { // Check if editing was enabled
        // If pointer lock is lost and we were editing, ensure editing state is cleaned up.
        // This might be redundant if exitFirstPersonMode or other flows already call cleanupIsolatedEditing.
        // cleanupIsolatedEditing(); 
        // Decided to keep cleanup only on explicit exit or error for now to avoid cycles if lock fails to acquire initially
    }
}
...
function handlePointerLockError() {
    ...
    isPointerLockRequested = false;
    if (isFirstPersonMode) {
        exitFirstPersonMode(); // If in FP mode, exit FP mode on error
    }
    if (isEditingEnabled) {
        cleanupIsolatedEditing(); // If editing was active, clean it up
    }
}

// Named function for context menu handling
function handleIsolatedContextMenu(event: MouseEvent) {
    if (isEditingEnabled || isFirstPersonMode) {
        event.preventDefault();
    }
}

// --- Camera Follow Utility for Agent ---
function setCameraToFollowAgent(agent: THREE.Object3D | null) {
    if (isFollowingAgent && followedAgent === agent) {
        // If already following this agent, stop following
        isFollowingAgent = false;
        followedAgent = null;
        console.log('[IsolatedTerrainViewer] Stopped following agent.');
        // Re-enable OrbitControls or other default controls
        if (orbitControls) orbitControls.enabled = true;
        // Assume other modes (FP/TP) will be re-enabled via their own logic or a state manager
    } else if (agent) {
        // Start following the provided agent
        isFollowingAgent = true;
        followedAgent = agent;
        console.log('[IsolatedTerrainViewer] Started following agent:', agent.name || agent.uuid || agent.id);
        // Disable other camera controls
        if (orbitControls) orbitControls.enabled = false;
        if (isFirstPersonMode) exitFirstPersonMode(); // Assuming exit functions disable controls
        if (isThirdPersonMode) exitThirdPersonMode(); // Assuming exit functions disable controls
    } else {
        // If called with null and not currently following, do nothing or handle as a stop command
        // This case might occur if the UI button is toggled off without a selected agent
         if (isFollowingAgent) {
            isFollowingAgent = false;
            followedAgent = null;
            console.log('[IsolatedTerrainViewer] Stopped following due to null agent.');
            if (orbitControls) orbitControls.enabled = true;
       }
    }
}

// Attach to window for UI panel access
if (typeof window !== 'undefined') {
    if (!window.isolatedTerrainViewer) window.isolatedTerrainViewer = {};
    window.isolatedTerrainViewer.setCameraToFollowAgent = setCameraToFollowAgent;
}

/**
 * Toggle the visibility of the Yuka AI Panel.
 * @param visible Optional boolean to force visibility state.
 */
export async function toggleYukaAIPanelVisibility(visible?: boolean) {
    // If the panel doesn't exist and we're trying to show it, create it
    if (!yukaAiPanelElement && (visible === undefined || visible === true)) {
        if (containerElement && scene && camera && renderer) {
            try {
                // Import the Yuka controller and initialize it if needed
                const { initYuka } = await import('../ai/yukaController');
                const { yukaEntityManager } = await import('../ai/yukaManager');
                ...
                console.log('Yuka AI Panel created and initialized.');
            } catch (error) {
                console.error('Failed to initialize Yuka AI Panel:', error);
                return;
            }
        } else {
            console.warn("Cannot create Yuka AI Panel: Required dependencies not available.");
            return;
        }
    } else if (!yukaAiPanelElement) {
        console.warn("IsolatedViewer: Yuka AI Panel element not found and cannot be created.");
        return;
    }

    const isCurrentlyVisible = yukaAiPanelElement.style.display !== 'none';
    const shouldBeVisible = visible !== undefined ? visible : !isCurrentlyVisible;

    if (shouldBeVisible) {
        yukaAiPanelElement.style.display = 'block';
        console.log("IsolatedViewer: Yuka AI Panel shown.");
    } else {
        yukaAiPanelElement.style.display = 'none';
        console.log("IsolatedViewer: Yuka AI Panel hidden.");
    }

     // We might also want to update the panel's internal state or controls if needed when shown/hidden
     // e.g., if (yukaAiPanelElement.updateDisplay) yukaAiPanelElement.updateDisplay();
}

// SpacetimeDB panel functions removed

// Attach to window for UI panel access
if (typeof window !== 'undefined') {
    if (!window.isolatedTerrainViewer) window.isolatedTerrainViewer = {};
    window.isolatedTerrainViewer.setCameraToFollowAgent = setCameraToFollowAgent;
    window.isolatedTerrainViewer.toggleYukaAIPanelVisibility = toggleYukaAIPanelVisibility;
    // SpacetimeDB panel function removed
    window.isolatedTerrainViewer.setWireframeEnabled = setWireframeEnabled;
    
    // Expose the renderer for SpacetimeDB panel
    window.isolatedTerrainViewer.getRenderer = getRenderer;
    window.isolatedTerrainViewer.getScene = getScene;
    window.isolatedTerrainViewer.getCamera = getCamera;
}

// Export function to toggle wireframe
export function setWireframeEnabled(enabled: boolean): void {
    console.log('[IsolatedTerrainViewer] Setting wireframe to:', enabled);

    if (enabled) {
        // Remove existing wireframe meshes before adding new ones
        removeWireframeMeshes();

        const chunks = currentChunkMeshesForCollision; // Use the module-level state for solid meshes
        const chunkKeys = Object.keys(chunks);
        console.log('[IsolatedTerrainViewer] Creating wireframe meshes for', chunkKeys.length, 'chunks.');

        if (chunks && chunkKeys.length > 0) {
            const wireframeMaterial = new THREE.MeshBasicMaterial({
                color: 0x00ff00, // Green wireframe
                wireframe: true,
                depthTest: true, // Ensure wireframe is drawn correctly over solid mesh
                depthWrite: false, // Don't obscure solid mesh
                transparent: true,
                opacity: 0.35,
            });

            Object.entries(chunks).forEach(([key, mesh]) => {
                console.log(`[IsolatedTerrainViewer] Processing chunk key ${key}:`, mesh);
                if (mesh && mesh.geometry) {
                    console.log(`[IsolatedTerrainViewer] Chunk ${key} has valid mesh and geometry.`);
                    try {
                        const wireframeMesh = new THREE.Mesh(mesh.geometry, wireframeMaterial);
                        console.log(`[IsolatedTerrainViewer] Wireframe mesh created for ${key}.`);
                        // Copy transformation from the solid mesh
                        wireframeMesh.position.copy(mesh.position);
                        wireframeMesh.rotation.copy(mesh.rotation);
                        wireframeMesh.scale.copy(mesh.scale);
                        wireframeMesh.name = `isolated_chunk_wireframe_${key}`;
                        wireframeMesh.renderOrder = 2; // Render on top of solid mesh (renderOrder 1)

                        const scene = getScene();
                        if (scene) {
                            scene.add(wireframeMesh);
                            currentWireframeMeshes[key] = wireframeMesh;
                             console.log(`[IsolatedTerrainViewer] Added wireframe mesh for chunk ${key} to scene. Total wireframe meshes tracked: ${Object.keys(currentWireframeMeshes).length}`);
                        } else {
                             console.warn(`[IsolatedTerrainViewer] Scene not available to add wireframe mesh for chunk ${key}.`);
                        }
                    } catch (error) {
                        console.error(`[IsolatedTerrainViewer] Error creating/adding wireframe mesh for chunk ${key}:`, error);
                    }
                }
            });
             console.log(`[IsolatedTerrainViewer] Finished processing chunks. Attempted to create wireframe meshes.`);
        } else if (chunkKeys.length === 0) {
             console.warn('[IsolatedTerrainViewer] No chunks found in currentChunkMeshesForCollision to create wireframes.');
        } else {
            console.warn('[IsolatedTerrainViewer] currentChunkMeshesForCollision is null or undefined when trying to create wireframes.');
        }
    } else {
        // Disable: Remove all wireframe meshes
        removeWireframeMeshes();
         console.log('[IsolatedTerrainViewer] Wireframe meshes removed.');
    }
}

// --- Helper function to remove all wireframe meshes ---
function removeWireframeMeshes() {
    console.log('[IsolatedTerrainViewer] Removing existing wireframe meshes.');
    const scene = getScene();
    if (!scene) {
         console.warn('[IsolatedTerrainViewer] Scene not available to remove wireframe meshes.');
         currentWireframeMeshes = {}; // Clear the map even if scene is missing
         return;
    }
    const removedCount = Object.keys(currentWireframeMeshes).length;
    Object.entries(currentWireframeMeshes).forEach(([key, mesh]) => {
        if (mesh && mesh.parent === scene) {
            ...
            console.log(`[IsolatedTerrainViewer] Removed wireframe mesh for chunk ${key}.`);
        } else {
            console.warn(`[IsolatedTerrainViewer] Wireframe mesh for ${key} not found in scene or already removed.`);
        }
    });
    currentWireframeMeshes = {}; // Clear the map
    console.log(`[IsolatedTerrainViewer] Finished removing ${removedCount} tracked wireframe meshes.`);
}

// Explicitly attach to window.isolatedTerrainViewer
if (typeof window !== 'undefined' && window.isolatedTerrainViewer) {
    window.isolatedTerrainViewer.setWireframeEnabled = setWireframeEnabled;
}