...
export function generateInitialChunks(
    ...
    for (let x = -initialRadius; x <= initialRadius; x++) {
        for (let z = -initialRadius; z <= initialRadius; z++) {
            const chunkKey = getChunkKeyY(x, 0, z);
            console.debug(`Generating initial chunk [${x}, 0, ${z}]...`);
            
            // Try loading first, otherwise generate
            let noiseMap = loadNoiseMap(x, z);
            if (!noiseMap) {
                console.debug(` -> Initial NoiseMap for [${x}, 0, ${z}] not found, generating...`);
                noiseMap = generateNoiseMap(x, 0, z, noiseLayers, seed);
            } else {
                 console.debug(` -> Loaded existing NoiseMap for [${x}, 0, ${z}].`);
            }
            ...
            (console as any).event(`🌱 Generated initial chunk [${x}, 0, ${z}]`);
        }
    }
